...
def write_instance_to_example_files(
    ...
    for (inst_index, instance) in enumerate(instances):
        ...
        if inst_index < 20:
            ...
            tf.logging.info(
                "tokens: %s" % " ".join([tokenization.printable_text(x) for x in instance.tokens])
            )
...
def create_training_instances(
    ...
    for input_file in input_files:
        with tensorflow.io.gfile.GFile(input_file, "r") as reader:
            while True:
...
def main(_):
    ...
    write_instance_to_example_files(
        instances, tokenizer, FLAGS.max_seq_length, FLAGS.max_predictions_per_seq, output_files
    )
